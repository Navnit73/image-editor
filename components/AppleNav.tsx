...
export function AppleNav({ currentLocale = "en", activeToolTitle }: AppleNavProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full">
      ...
      <nav className="h-[44px] bg-[#000000] text-white flex items-center justify-center px-4 md:px-8 border-b border-white/10">
        <div className="w-full max-w-[1024px] flex items-center justify-between font-nav-link text-[12px] tracking-[-0.12px]">
          {/* Apple Logo Icon */}
          <Link href={`/${localeKey}`} className="hover:opacity-70 transition-opacity flex items-center gap-1.5" aria-label="Apple Image Studio">
            ...
            <span className="font-semibold text-[13px] tracking-tight">Studio</span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-6 text-[#cccccc]">
            ...
            <Link href={`/${localeKey}/${localeKey === "pt" ? "cortar-imagem" : localeKey === "de" ? "bild-zuschneiden" : "image-cropper"}`} className="hover:text-white transition-colors">{i18n.crop}</Link>
            <Link href={`/${localeKey}/${localeKey === "pt" ? "redimensionar-imagem" : localeKey === "de" ? "bildgroesse-aendern" : "resize-image"}`} className="hover:text-white transition-colors">{i18n.resize}</Link>
            <Link href={`/${localeKey}/${localeKey === "pt" ? "compactar-imagem" : localeKey === "de" ? "bild-komprimieren" : "compress-image"}`} className="hover:text-white transition-colors">{i18n.compress}</Link>
            <Link href={`/${localeKey}/${localeKey === "pt" ? "foto-passaporte" : localeKey === "de" ? "passfoto-generator" : "passport-photo-maker"}`} className="hover:text-white transition-colors">{i18n.passport}</Link>
          </div>
      ...
      {/* Sub-Nav Frosted - 52px height */}
      <div className="h-[52px] frosted-glass border-b border-[#000000]/10 flex items-center justify-center px-4 md:px-8">
        <div className="w-full max-w-[1024px] flex items-center justify-between">
          <span className="font-tagline text-[21px] font-semibold text-[#1d1d1f]">
            {activeToolTitle || i18n.subnavTitle}
          </span>
          <div className="flex items-center space-x-4">
            <a href="#editor-container" className="btn-apple-primary text-[14px] py-1.5 px-4 font-normal">
              {i18n.tryFree}
            </a>
          </div>
        </div>
